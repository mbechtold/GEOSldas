...
module enkf_types
  ...
  type :: obs_param_type
     ...
     logical                       :: errstd_file  ! read obs err stdv from file yes/no?
     character(200)                :: path_errstd  ! path to files with obs err std
...
contains
  ...
  subroutine write_obs_param(unitnumber, N_obs_param, obs_param)
    ...
    do i=1,N_obs_param
       ...
       write (unitnumber,       *) obs_param(i)%errstd_file
       write (unitnumber,'(202A)') "'" // trim(obs_param(i)%path_errstd) // "'"
