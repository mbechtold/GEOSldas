...
contains
  ...
  subroutine read_std_obs_SMOS( N_catd, this_obs_param,      &
       tile_coord, tile_grid_d,                     &
       N_tile_in_cell_ij, tile_num_in_cell_ij,      &
       std_SMOS_data) !, std_SMOS_lon, std_SMOS_lat   )

    ! TO BE DONE: send std_SMOS_lon, std_SMOS_lat as output and *verify* against
    ! output obtained after reading the SMOS obs

    ! Reader for obs error in preprocessed SMOS Tb and soil moisture (SM).
    ! Obs error includes both measurement error and representativeness error.
    !
    ! IMPORTANT: this approach does *NOT* account for time-variable
    ! (i) radiometric error which is read in for regular SMOS data, nor the
    ! (ii) fitting error which is read in for the fitted SMOS data.
    ! Consequently, the obs error is NOT adjusted for the obs position in a swath.
    !
    ! 5 Sep 2014, De Lannoy
    !
    ! 1 Jun 2018, Gruber
    !             modified: one errstd value for the whole year instead of monthly
    ! --------------------------------------------------------------
    ...
    real,    intent(out), dimension(N_catd) :: std_SMOS_data
!    real,    intent(out), dimension(N_catd) :: std_SMOS_lon, std_SMOS_lat
    real,    dimension(N_catd) :: std_SMOS_lon, std_SMOS_lat
    ...
    ! avoid ambiguous assignment of M09 model tile within M36 obs grid cell
    ! (center of M36 grid cell is equidistant from at least two M09 model
    !  tiles) -- copied from reichle, 23 Aug 2013
    ...
    integer,   parameter :: unitnum = 11

    integer              :: ii, i, j, k, n, istat
    integer              :: ind_tile, ind_angle
    integer              :: N_std_obs, N_ang

    logical              :: SM_files, hpol
    ...
    integer,        dimension(N_catd)           :: N_std_obs_in_tile
    integer,        dimension(:),   allocatable :: start_time, end_time

    integer              :: Asc_flag, N_min

    real,           dimension(:),   allocatable :: tmp_lon, tmp_lat, tmp_ang
    real,           dimension(:),   allocatable :: tmp_std_obs

    integer,        dimension(:),   allocatable :: tmp_tile_num
    integer,        dimension(:),   allocatable :: tmp_file_ind

    character(len=*), parameter :: Iam = 'read_std_obs_SMOS'
    character(len=400) :: err_msg

    ! read error for soil moisture or brightness temperature files?
    ...
    elseif (index(this_obs_param%descr,'_Tb') /= 0) then
       ...
       elseif (index(this_obs_param%descr,'_L1C') /= 0) then
    ...
    ! ---------------------------
    !
    ! find month from time stamp for obs error file
    ! TO BE DONE: this may need temporal interpolation
    !

    fname = trim(this_obs_param%path_errstd) // '/' // &
            trim(ftag) // orbit_tag //'.bin'


    open(unitnum, file=trim(fname),form='unformatted',convert='big_endian',status='old', &
               access='SEQUENTIAL', iostat=istat)


    if (istat/=0) then

       err_msg = 'could not open file'// trim(fname)
       call ldas_abort(LDAS_GENERIC_ERROR, Iam, err_msg)
    ...
    read(unitnum) Asc_flag, N_min
    read(unitnum)
    read(unitnum)
    read(unitnum) N_std_obs, N_ang

    if (logit) write (logunit,*) '  Asc_flag   = ', Asc_flag
    !if (logit) write (logunit,*) '  start_time = ', start_time( :)
    !if (logit) write (logunit,*) '  end_time   = ', end_time(   :)
    if (logit) write (logunit,*) '  N_std_obs  = ', N_std_obs
    if (logit) write (logunit,*) '  N_ang      = ', N_ang

    allocate(tmp_ang(N_ang))
    allocate(tmp_lon(N_std_obs))
    allocate(tmp_lat(N_std_obs))
    allocate(tmp_std_obs(N_std_obs))

    read(unitnum) tmp_ang
    read(unitnum) tmp_lon
    read(unitnum) tmp_lat

    if (SM_files) then

      ! read error for SMOS SM

      read(unitnum) tmp_std_obs

    else

      ! read error for SMOS Tb
    ...
    do n=1,N_std_obs
       ...
       if (tmp_std_obs(n) >= 0) then

          j = j +1

          tmp_std_obs(j) = tmp_std_obs(n)
          tmp_lon(j)     = tmp_lon(n)
          tmp_lat(j)     = tmp_lat(n)
     ...
     ! -------------------------------------------------
     !
     ! map std_obs to administrating tiles
     !
     ! for each observation error stdv
     !     a) determine grid cell that contains lat/lon
     !     b) determine tile within grid cell that contains lat/lon

     if (N_std_obs>0) then

             allocate(tmp_tile_num(N_std_obs))
             ...
             call get_tile_num_for_obs(N_catd, tile_coord,                 &
                  tile_grid_d, N_tile_in_cell_ij, tile_num_in_cell_ij,     &
                  N_std_obs, tmp_lat+tmp_shift_lat, tmp_lon+tmp_shift_lon, &
                  this_obs_param, tmp_tile_num )
             ...
             do i=1,N_std_obs
                ...
                if (ind_tile>0) then         ! this step eliminates obs outside domain
                   ...
                   std_SMOS_data(ind_tile) = std_SMOS_data(ind_tile) + tmp_std_obs(i)
                   std_SMOS_lon( ind_tile) = std_SMOS_lon( ind_tile) + tmp_lon(i)
                   std_SMOS_lat( ind_tile) = std_SMOS_lat( ind_tile) + tmp_lat(i)

                   N_std_obs_in_tile(ind_tile) = N_std_obs_in_tile(ind_tile) + 1
             ...
             do i=1,N_catd
                ...
                if (N_std_obs_in_tile(i)>1) then

                   tmpreal = real(N_std_obs_in_tile(i))

                   std_SMOS_data(i) = std_SMOS_data(i)/tmpreal
                   std_SMOS_lon( i) = std_SMOS_lon( i)/tmpreal
                   std_SMOS_lat( i) = std_SMOS_lat( i)/tmpreal

                elseif (N_std_obs_in_tile(i)==0) then

                   std_SMOS_data(i) = this_obs_param%nodata
                   std_SMOS_lon( i) = this_obs_param%nodata
                   std_SMOS_lat( i) = this_obs_param%nodata
    ...
    deallocate(tmp_lon)
    deallocate(tmp_lat)
    deallocate(tmp_std_obs)

  if (logit) write (logunit,*) 'read_std_obs_SMOS(): done.'

  end subroutine read_std_obs_SMOS
...
! <Alexander Gruber : 2021-09-18> Implemented for reading spatially variable observation perturbations
      if (found_obs .and. this_obs_param%errstd_file) then
        call read_std_obs_SMOS(                          &
             N_catd, this_obs_param,          &
             tile_coord, tile_grid_d,                    &
             N_tile_in_cell_ij, tile_num_in_cell_ij,     &
             tmp_std_obs )
      end if
